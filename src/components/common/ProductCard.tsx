...
export default function ProductCard({
  ...
  return (
    <div className="w-[330px] relative group">
      <ImageWrapper
        className={twMerge("h-[400px] cursor-pointer", className)}
        src={src}
        alt={alt}
        imgClassName={imgClassName}
      />
      <button className="absolute bottom-24 left-4 right-4 bg-white text-black py-3 px-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300 cursor-pointer">
        Add to Cart
      </button>
      <div className="pt-3">
        <div className="px-0.5">
          <Flex className={"justify-between"}>
            <h5 className="text-gray-500 text-sm cursor-pointer">{category}</h5>
            <SvgWrapper>
              <svg
                width="16"
                height="14"
          ...
          <h4 className="cursor-pointer">{productName}</h4>
          {price !== undefined ? (
